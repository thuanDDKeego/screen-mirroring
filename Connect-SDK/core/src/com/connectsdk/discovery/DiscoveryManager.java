...
public class DiscoveryManager implements ConnectableDeviceListener, DiscoveryProviderListener, ServiceConfigListener {
    ...
    public void registerDefaultDeviceTypes(HashMap<String, String> devicesList) {
        for (HashMap.Entry<String, String> entry : devicesList.entrySet()) {
            String key = entry.getKey();
            String value = entry.getValue();
            try {
    ...
    /**
     * sofi custom method
     * load devices from specific devices list
     *
     * @param devices
     */
    public void registerDeviceTypes(List<String> devices) {
        final HashMap<String, String> devicesList = DefaultPlatform.getDeviceServiceMap();
        ...
        for (String device : devices) {
            if (devicesList.containsKey(device)) {
                String value = devicesList.get(device);
                try {
                    registerDeviceService((Class<DeviceService>) Class.forName(device), (Class<DiscoveryProvider>) Class.forName(value));
                } catch (ClassNotFoundException e) {
                    e.printStackTrace();
                }
            }
